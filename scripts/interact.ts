...
async function main() {
    ...
    if (!TOKEN /*|| TOKEN === "0x5fbdb2315678afecb367f032d93f642f64180aa2"*/) throw new Error("Set TOKEN in scripts / interact.ts");
const { abi } = await artifacts.readArtifact("CampusCredit");
    ...
    const balances = async (label: string) => {
        ...
        const b2 = await publicClient.readContract({
            address: getAddress(TOKEN), abi,
            functionName: "balanceOf", args: [getAddress(acct2)]
        });
    ...
    const tx1 = await wallet.writeContract({
        ...
        args: [getAddress(acct2), parseUnits("100", DECIMALS)],
    ...
    const tx2 = await wallet.writeContract({
        ...
        abi,
        functionName: "transfer",
        args: [getAddress(acct2), parseUnits("50", DECIMALS)],
    ...
    const tx3 = await wallet.writeContract({
        ...
        args: [getAddress(acct2), parseUnits("25", DECIMALS)],
